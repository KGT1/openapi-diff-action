...
| `circular-ref-mode` | No | `replace-allof` | Circular reference handling: `replace-allof` (deterministic) or `flatten` |
...
**`replace-allof` (default)**: Replaces `"allOf"` with `"oneOf"` using sed. Works around the bug deterministically since openapi-diff handles `oneOf` circular refs correctly but not `allOf`.

**`flatten`**: Uses `oasdiff flatten`. Warning: produces non-deterministic output (same input → different nullable/type fields on each run), causing false positives.
